...
function computeMetrics(values) {
  ...
  // 7일 전 시점의 종가와 200일선 — '최근 7일 내 200일선 돌파' 판정용
  // closes[0]=오늘. 7거래일 전 = closes[7], 그 시점의 200일선 = closes[7..206] 평균
  let crossedMA200_7d = false, aboveMA200Now = false;
  if (closes.length >= 207 && ma200 != null) {
    aboveMA200Now = close > ma200;
    const close7 = closes[7];
    const ma200_7ago = avg(closes.slice(7, 207));
    const above7ago = close7 > ma200_7ago;
    // 지금은 200일선 위인데, 7일 전엔 200일선 이하였다 → 최근 7일 내 상향 돌파
    crossedMA200_7d = aboveMA200Now && !above7ago;
  } else if (ma200 != null) {
    aboveMA200Now = close > ma200;
  }
  ...
  return { closes, highs, lows, vols, close, prevClose, ma20, ma50, ma150, ma200, ma200_20ago, high52, high30, high30_prev, atr, pull, dayChg, rsRaw, crossedMA200_7d, aboveMA200Now };
}
...
function scoreFromMetrics(m, idxRef, rsPercentile) {
  ...
  return {
    ...
    // 200일선 관련: 현재 위 여부 + 최근 7일 내 상향 돌파 여부
    aboveMA200: m.aboveMA200Now ? 1 : 0,
    crossedMA200_7d: m.crossedMA200_7d ? 1 : 0,
  };
}
...
function trendChannel(values, lookback = 90) {
  const raw = values.slice(0, lookback).reverse(); // 오름차순(과거→현재)
  if (raw.length < 20) return null;
  // Twelve Data는 문자열로 주므로 숫자 변환 필수
  const slice = raw.map((v) => ({
    high: parseFloat(v.high), low: parseFloat(v.low), close: parseFloat(v.close),
  })).filter((v) => !isNaN(v.close) && !isNaN(v.high) && !isNaN(v.low));
...
function multiTrend(values) {
  const closesDesc = values.map((v) => parseFloat(v.close)).filter((n) => !isNaN(n));
