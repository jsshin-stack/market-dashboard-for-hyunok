...
### 외부 스케줄러 설정 (무료) — 장 마감 후 시간대에만
Vercel Hobby Cron은 하루 1회뿐이라, 이어받기를 돌리려면 외부 무료 스케줄러를 씁니다. **종일 5분마다 돌리면 Twelve Data 일일 한도(800회)를 금세 초과하므로, 미국 장 마감 후 ~30분 구간에만 5분 간격으로 돌리세요.**
1. https://cron-job.org 무료 가입
2. Create cronjob → URL에 `https://(내사이트)/api/cron-refresh` 입력
3. 실행 주기(Advanced/Custom 크론식): 미국 동부 장 마감(16:00 ET) 후 약 30분만. 예: 한국시간 기준 오전 5~6시(서머타임)/6~7시 사이 5분 간격. 약 6회 호출이면 110종목이 다 채워집니다(이어받기).
4. 한 번 다 채워지면 이후 호출은 "이미 완료"로 즉시 스킵되어 한도를 거의 안 씁니다.
- 한도 초과 등으로 Twelve Data가 응답을 못 주면, 수집 API는 한도를 더 낭비하지 않고 즉시 중단합니다(`skipped: true` 반환).
- Twelve Data 무료 한도는 UTC 자정(한국시간 오전 9시경)에 리셋됩니다.
